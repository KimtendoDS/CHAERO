...
const RouteDetail = ({ setStep }: { setStep: (s: number) => void }) => {
  ...
  const topThreeWeeks = [
    ...
    },
    {
      ...
      koLabel: '2주차',
      title: '을지로 감성 아카이브',
      subTitle: '레트로한 공간에서 즐기는 깊은 밤의 정취',
      tags: ['LP바', '레트로', '노포'],
      moodKey: 'VIBE',
      metrics: { schedule: '7일', dist: '10.2km', time: '5시간' },
    },
    {
      id: 3,
      koLabel: '3주차',
      title: '북촌-삼청 전통 투어',
      subTitle: '고즈넉한 한옥 사이로 발견하는 서울의 어제와 오늘',
      tags: ['한옥', '전통', '산책'],
      moodKey: 'CLASSIC',
      metrics: { schedule: '7일', dist: '8.7km', time: '3.5시간' },
    },
  ];
  ...
  const getCalculatedDate = (index: number) => {
    ...
    return `${baseDate.getFullYear()}.${String(baseDate.getMonth() + 1).padStart(2, '0')}.${String(
      baseDate.getDate(),
    ).padStart(2, '0')} (${dayNames[baseDate.getDay()]})`;
  };
      ...
      items: [
        {
          ...
          duration: '3h',
          title: '경복궁 투어',
          description: '조선의 역사를 품은 궁궐 탐방',
          type: 'activity',
          icon: '🏛️',
          mood: 'CLASSIC',
          spots: '경복궁, 국립민속박물관',
          price: '무료',
          reason: '전통 문화를 깊이 있게 경험',
        },
        {
          id: 2,
          time: '12:30',
          duration: '1h',
          title: '점심 - 토속촌 삼계탕',
          description: '경복궁 인근 유명 맛집',
          type: 'meal',
          icon: '🍲',
          mood: 'CLASSIC',
          spots: '토속촌',
          price: '2만원',
          reason: '동선상 가까운 전통 한식',
        },
        {
          id: 3,
          time: '14:00',
      ...
      items: [
        {
          ...
          duration: '2h',
          title: '성수동 카페 투어',
          description: '핫플 카페 3곳 방문',
          type: 'activity',
          icon: '☕',
          mood: 'TRENDY',
          spots: '대림창고, 어니언',
          price: '1.5만원',
          reason: '젊은 감성의 트렌디한 공간',
        },
        {
          id: 5,
          time: '13:00',
  ...
  const handleItemReorder = (dayIndex: number, newItems: ScheduleItem[]) => {
    setSchedule((prev) => prev.map((d, i) => (i === dayIndex ? { ...d, items: newItems } : d)));
  };
  ...
  const replacementOptions: Recommendation[] = [
    {
      ...
      matchScore: 93,
      imageUrl: '',
      description: '전통 찻집에서 한국 차 문화 체험',
      type: 'activity',
      icon: '🍵',
      mood: 'CLASSIC',
      spots: '익선다원, 차마시는뜰',
      price: '2만원',
      reason: '경복궁 동선과 연결, 전통 문화 심화',
      isSponsored: true, // SPONSORED
    },
    {
      id: 102,
      targetDay: 2,
      ...
      duration: '2h',
      title: '을지로 감성 바 투어',
      brandName: 'Vinyl & Plastic',
      category: '바/라운지',
      photoAlt: 'LP가 가득한 레트로 감성 바 인테리어',
      rating: 4.6,
      reviewCount: 1840,
      distanceKm: 2.1,
      area: '을지로',
      ...
      matchScore: 89,
      imageUrl: '',
      description: '레트로 감성 LP바와 루프탑',
      type: 'activity',
      icon: '🎵',
      mood: 'VIBE',
      spots: 'Vinyl&Plastic, 을지다락',
      price: '3만원',
      reason: '빈티지 감성, 성수와 다른 느낌',
    },
    {
      id: 103,
      targetDay: 2,
  ...
  const handleSkip = () => {
    if (currentCard) {
      setExcludedIds(prev => [...prev, currentCard.id]);
    }
    const len = filteredOptions.length || 1;
    if (currentRecommendationIndex >= len - 1) {
      // 마지막 카드였으면 인덱스 유지 (새로고침 버튼 표시)
      setCurrentRecommendationIndex(len - 1);
    } else {
      setCurrentRecommendationIndex((p) => p + 1);
    }
    x.set(0);
  };

  const handleAction = (item: Recommendation | null) => {
    console.log('🔵 handleAction called', { isProcessing: isProcessing.current, addMode, selectedSlot });
    
    if (!item) {
      console.log('❌ No item');
      return;
    }
    if (!addMode && !selectedSlot) {
      console.log('❌ No mode or slot');
      return;
    }
    
    // Strict Mode 대응: 이미 처리중이면 무시
    if (isProcessing.current) {
      console.log('⚠️ BLOCKED - Already processing!');
      return;
    }
    ...
    if (currentAddMode) {
      ...
      console.log('➕ Adding item:', added.id);
      ...
      setSchedule((currentSchedule) => {
        ...
        newSchedule[dayIndex] = {
          ...newSchedule[dayIndex],
          items: [...newSchedule[dayIndex].items, added].sort((a, b) => 
  ...
  const handleCardDragEnd = (event: any, info: PanInfo) => {
    if (Math.abs(info.offset.x) > 100) {
      if (info.offset.x > 0) {
        // 우측 드래그 = 하트 클릭 (일정 추가)
        handleAction(currentCard);
      } else {
        // 좌측 드래그는 스킵 (제외 목록에 추가)
        handleSkip();
      }
    } else {
      x.set(0);
  ...
  const DayBlock = ({ day, dayIndex }: { day: ScheduleDay; dayIndex: number }) => {
        ...
        <div style={{ 
          ...
          >
            <div>
              ...
              <div style={{ fontSize: '11px', color: pearlColor, fontWeight: '700' }}>
                {getCalculatedDate(dayIndex)} · {day.items.length}개 일정
              </div>
            </div>
          ...
          {isExpanded && (
            ...
            >
              <Reorder.Group
                axis="y"
                values={day.items}
                onReorder={(newItems) => handleItemReorder(dayIndex, newItems)}
              ...
              >
                {day.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {/* 빨간 삭제 배경 */}
                      <div onClick={() => deleteScheduleItem(dayIndex, item.id)} style={{
                        position: 'absolute',
                        right: 0,
                        top: 0,
                        bottom: 0,
                      ...
                      >
                        <div
                          ...
                          style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', flex: 1, cursor: 'pointer' }}
                        >
                          <span style={{ fontSize: '28px' }}>{item.icon}</span>
                          <div>
                            <div style={{ fontSize: '11px', fontWeight: '800', color: style.color, marginBottom: '2px' }}>
              ...
              <div style={{ padding: '20px' }}>
                ...
                >
                  ...
                  <span style={{ fontSize: '13px', fontWeight: '800', color: 'rgba(255,255,255,0.4)' }}>
                    일정 추가하기
                  </span>
                </motion.div>
              </div>
  ...
  return (
    <div style={{ flex: 1, background: '#020306', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <style jsx global>{`
      ...
      <div style={{ 
        ...
        <div style={{ padding: '20px 20px 10px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <h1 style={{ fontSize: '26px', fontWeight: '900', color: '#fff', margin: '0 0 8px 0', lineHeight: 1.2 }}>
            당신의{' '}
            <span style={{ background: najeonGrad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              맞춤 여행
            </span>
          </h1>
          <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', margin: 0, lineHeight: 1.5 }}>
            AI가 분석한 일정이에요. 좌우로 드래그해 순서를 바꿔보세요!
          </p>
        </div>
        ...
        <div style={{ position: 'relative', overflow: 'hidden', borderRadius: '24px' }}>
          <motion.div
            ...
            onDragEnd={(e, info) => {
              const threshold = 50;
              if (info.offset.x < -threshold && currentSlide < topThreeWeeks.length - 1) {
                setCurrentSlide(currentSlide + 1);
              } else if (info.offset.x > threshold && currentSlide > 0) {
                setCurrentSlide(currentSlide - 1);
              }
            }}
            animate={{ x: `${-currentSlide * (100 / topThreeWeeks.length)}%` }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            style={{ display: 'flex', width: `${topThreeWeeks.length * 100}%`, cursor: 'grab' }}
          >
            {topThreeWeeks.map((bundle) => {
              const style = vibeStyles[bundle.moodKey];
              return (
                <div key={bundle.id} style={{ width: `${100 / topThreeWeeks.length}%`, padding: '0 4px' }}>
                  <div
                    style={{
                      background: 'rgba(255,255,255,0.02)',
                      border: glassBorder,
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', marginBottom: '12px', alignItems: 'center'  }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', margin: '0 0 10px 0', lineHeight: 1.3 }}>
                          {bundle.subTitle}
                        </p>
                      </div>
                      <div
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Calendar size={11} color={pearlColor} />
                        <span style={{ fontSize: '10px', color: pearlColor, fontWeight: '800' }}>{bundle.metrics.schedule}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Map size={11} color={pearlColor} />
                        <span style={{ fontSize: '10px', color: pearlColor, fontWeight: '800' }}>{bundle.metrics.dist}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Clock size={11} color={pearlColor} />
                        <span style={{ fontSize: '10px', color: pearlColor, fontWeight: '800' }}>{bundle.metrics.time}</span>
                      </div>
                    </div>
      ...
      <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '0 20px 140px 20px' }}>
        <Reorder.Group 
          axis="y" 
          values={schedule} 
        ...
        >
          {schedule.map((day, dayIndex) => (
            <DayBlock key={day.id} day={day} dayIndex={dayIndex} />
          ))}
        </Reorder.Group>
      </div>
          ...
          <>
            ...
            <motion.div
              ...
              style={{
                ...
                left: 0,
                right: 0,
                background: 'linear-gradient(to bottom, #111418, #05070A)',
            ...
            >
              ...
              <div style={{ height: '420px', position: 'relative', display: 'flex', justifyContent: 'center' }}>
                {currentCard ? (
                  <motion.div
                    style={{ x, rotate, opacity, width: '100%', maxWidth: '380px', position: 'relative' }}
                    drag="x"
                    dragConstraints={{ left: 0, right: 0 }}
                  ...
                  >
                    ...
                    <motion.div
                      style={{
                        ...
                        fontWeight: '900',
                        zIndex: 20,
                        opacity: successHintOpacity,
                        boxShadow: '0 8px 20px rgba(94,234,212,0.35)',
                      }}
                    >
                      ♥ PICK
                    </motion.div>
                    ...
                    >
                      ...
                      <div style={{ padding: '24px' }}>
                        ...
                        <div style={{ fontSize: '22px', fontWeight: '900', color: '#fff', textAlign: 'center', marginBottom: '8px' }}>
                          {currentCard.brandName || currentCard.title}
                        </div>
                        <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', textAlign: 'center', marginBottom: '20px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap', justifyContent: 'center' }}>
                          <div style={{
                            ...
                            ★ {currentCard.rating?.toFixed(1) || '4.7'}
                          </div>
                          <div style={{
                            padding: '8px 12px',
                            borderRadius: '12px',
                        ...
                        <div style={{
                          ...
                          <div style={{ fontSize: '11px', fontWeight: '900', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>
                            추천 이유
                          </div>
                  ...
                  <div style={{ textAlign: 'center', paddingTop: 80, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
                    ...
                    <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: '16px', fontWeight: '700' }}>
                      모든 추천을 확인했어요!
                    </div>
                    <motion.button
                      ...
                      style={{
                        ...
                        fontWeight: '900',
                        cursor: 'pointer',
                        boxShadow: '0 8px 20px rgba(94,234,212,0.3)'
                      }}
                    >
                      다시 보기
                    </motion.button>
              ...
              <div style={{ display: 'flex', gap: '25px', justifyContent: 'center', marginTop: '30px' }}>
                ...
                <motion.button
                  ...
                  style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    boxShadow: '0 10px 25px rgba(94,234,212,0.30)',
                    border: 'none',
                    cursor: 'pointer',
                  }}
                >
                  <Heart size={35} fill="#000" />
                </motion.button>
              </div>
            </motion.div>
          </>
      ...
      {/* 하단 길 안내 시작 버튼 - 아이콘 제거 및 글로우 효과 적용 */}       
      {!selectedSlot && !addMode && (
        <div
          style={{
            position: 'fixed',
            ...
            zIndex: 10,
            background: 'linear-gradient(to top, #020306 80%, transparent)',
          }}
        >
          <motion.button
            whileTap={{ scale: 0.96 }}
            // !!!!!!!!!!!!!!!!!!! [수정] 클릭 시 다음 스텝(MapPath)으로 이동
            ...
            style={{
              ...
              border: 'none',
              color: '#000',
              fontWeight: '950', // 글자 굵게
              fontSize: '18px',
              cursor: 'pointer',
              // !!!!!!!!!!!!!!!!!!! [수정] 아이콘 빼고 빛나는 효과(Glow) 추가
